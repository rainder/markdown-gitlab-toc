...
function *generateToc () {
  ...
  function excludeFiles(item) {
    return !commander.exclude || !item.match(new RegExp(commander.exclude));
  }
...
function buildATree(files) {
  ...
  for (let file of files) {
    ...
    objectDeep.set(tree, file.parts.slice(0, -1).map(replaceDots).join('.'), obj);
  }
...
function convertToMarkdownTOC(tree) {
  ...
  (function traverse(tree, level) {
    ...
    for (let key in tree) {
      ...
      if (item.hasOwnProperty(dataSymbol)) {
        let value = key || item[dataSymbol].header;
        line += `[${value}](${item[dataSymbol].path})`;
      } else {
        line += key;
      }
